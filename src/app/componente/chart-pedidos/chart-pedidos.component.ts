...
export class ChartPedidosComponent {
  ...
  public periodoForm = this.formBuilder.group({
    ...
    public barChartOptions = {
      ...
      ngOnInit(): void {
        this.reportesService.getPedidosPorPeriodo({desde:'',hasta:''}).subscribe(data => {
      ...
      buscarPorPeriodo(){
        ...
        this.reportesService.getPedidosPorPeriodo(inputs).subscribe(data => {
          this.barChartData[0].data = []
          this.barChartLabels = []
          let array = Object.values(data)
            array.forEach((valor,indice) =>{
